...
merge outer inner
  ...
  where
    innerHoriz = spf inner innerEntry
    innerVert = lpf inner innerEntry
    innerEntry =
      case find (\x -> null (inn outer x) && null (out outer x)) (nodes outer) of
        Just x -> x
    outerHoriz = spf outer outerEntry
    outerVert = lpf outer outerEntry
    outerEntry =  x
      where
        x =
          last $
